...
def test_get_project_root():
    assert get_project_root().name == "telemetry-airflow"
...
@pytest.mark.parametrize(("test_input", "expected"),
                         [
                             (("# example comment on first line\n"
                               ...
                               "# Rest of Dockerfile"), "slim-2.8.2-python3.11"),
...
def test_get_airflow_version_exists():
    ...
    mock_parse_airflow_version = patch("plugins.version_endpoint.parse_airflow_version",
                                       return_value="2.8.2")
    ...
    with (mock_project_root, mock_parse_airflow_version, mock_open_file, mock_is_file,
          mock_exists):
...
def test_get_dockerflow_version_exists():
    mock_project_root = patch("plugins.version_endpoint.get_project_root",
                              return_value=Path("/mock/path"))
    mock_open_file = patch("builtins.open", mock_open(read_data=json.dumps({
        "build": "12345",
        "commit": "abcdef",
