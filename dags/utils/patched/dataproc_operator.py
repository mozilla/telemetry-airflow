...
https://github.com/apache/airflow/blob/providers-google/5.0.0/airflow/providers/google/cloud/operators/dataproc.py

I originally created these patch files to fix a potential issue with
DataprocSubmit(Py)SparkJobOperators because the dataproc_v1beta2
services/apis/etc were deprecated and removed from the google-cloud-dataproc
package already as of 2.6.0 and 3.0.0 and the apache-airflow-providers-google
(v6.0.0 latest) hasn't been updated to reflect this.

This ended up not being the issue, but I am keeping these patched versions around
because they make it easier to troubleshoot (adding prints), and google will
eventually deprecate the dataproc_v1beta2 apis requiring us to upgrade airflow providers
and redeploy. By keeping these patched versions, we can simply modify utils/dataproc.py
to point to these patched versions.

This file replaces all instances of dataproc_v1beta2 with dataproc_v1
and imports our own patched versions of DataprocHook/DataProcJobBuilder
which will have replaced refs of dataproc_v1beta2 as well.
""" 
...
# These will be import from our patched version instead
# from airflow.providers.google.cloud.hooks.dataproc import DataprocHook, DataProcJobBuilder
from utils.patched.dataproc_hook import (
    DataprocHook,
    DataProcJobBuilder,
)
