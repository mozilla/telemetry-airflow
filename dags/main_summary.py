...
clients_last_seen_export = SubDagOperator(
    subdag=export_to_parquet(
        ...
        arguments=[
            ...
            "--select",
            "cast(log2(days_seen_bits & -days_seen_bits) as integer) as days_since_seen",
            "cast(log2(days_visited_5_uri_bits & -days_visited_5_uri_bits) as integer) as days_since_visited_5_uri",
            "cast(log2(days_opened_dev_tools_bits & -days_opened_dev_tools_bits) as integer) as days_since_opened_dev_tools",
            "cast(log2(days_created_profile_bits & -days_created_profile_bits) as integer) as days_since_created_profile",
            "*"
        ],
        parent_dag_name=dag.dag_id,
