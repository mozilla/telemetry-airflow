...
with DAG(
    ...
    def complete_backfill(backfill):
        ...
        def prepare_slack_complete_message(entry):
            ...
            return f"{watcher_text} :hourglass_flowing_sand: Completing backfill of `{entry['qualified_table_name']}` has started. A snapshot of the current production data will be kept as a backup.  You will receive another notification once the completing step is done."
