...
# TODO - delete this job after redeploying mozagg prod, and switching DNS to point to the lean db
EXPORT_TO_AVRO = True

default_args = {
    "owner": "frank@mozilla.com",
    "depends_on_past": True,
    "start_date": datetime(2020, 02, 20),
    "email": [
...
dag = DAG(
    "prerelease_telemetry_aggregates_new",
...
prerelease_telemetry_aggregate_view_dataproc = SubDagOperator(
    ...
    subdag=moz_dataproc_pyspark_runner(
        ...
        dag_name=task_id,
        job_name="prerelease_aggregates_new",
        cluster_name="prerelease-telemetry-aggregates-new-{{ ds_nodash }}",
        idle_delete_ttl="600",
        num_workers=10,
        ...
        py_args=[
            ...
            "root",
            "--postgres-pass",
            "{{ var.value.mozaggregator_lean_postgres_pass }}",
            "--postgres-host",
            "{{ var.value.mozaggregator_lean_postgres_host }}",
            "--postgres-ro-host",
            "{{ var.value.mozaggregator_lean_postgres_ro_host }}",
            "--num-partitions",
            str(10 * 32),
        ]
        + (
            ...
            if not EXPORT_TO_AVRO
                ...
                "avro",
                "--avro-prefix",
                "gs://moz-fx-data-derived-datasets-parquet-tmp/avro/mozaggregator/prerelease-new/moz-fx-data-shared-prod",
            ]
        ),
...
if EXPORT_TO_AVRO:
    gke_command(
        ...
        command=[
            ...
            "moz-fx-data-shared-prod:tmp",
            "gs://moz-fx-data-derived-datasets-parquet-tmp/avro/mozaggregator/prerelease-new",
            "main_v4",
            "'nightly', 'beta'",
            "{{ ds }}",
        ],
    ...
    GoogleCloudStorageDeleteOperator(
        ...
        prefix="avro/mozaggregator/prerelease-new/moz-fx-data-shared-prod/{{ ds_nodash }}/main_v4",
    ...
    GoogleCloudStorageDeleteOperator(
        ...
        bucket_name="moz-fx-data-derived-datasets-parquet-tmp",
        prefix="avro/mozaggregator/prerelease-new/moz-fx-data-shared-prod/{{ ds_nodash }}/saved_session_v4",
        google_cloud_storage_conn_id=gcp_conn.gcp_conn_id,
        dag=dag
    ).set_upstream(prerelease_telemetry_aggregate_view_dataproc)
